...
export default function DetailVisualization({ navigation, route }) {
    ...
    useEffect(() => {
        if (modeSelected === 'MONTH') { 
            axiosAdafruit.get(`${typeSensor}/data/chart?hours=720&resolution=480`)
                .then((res) => {
            ...
            axiosAdafruit.get(`${typeSensor}/data/chart?hours=24&resolution=30`)
                .then((res) => {
                    setData(res.data.data)
                    const resValue = res.data.data.map(item => Number(item[1]))
                    setDataTemp(resValue)
            ...
            axiosAdafruit.get(`${typeSensor}/data/chart?hours=168&resolution=240`)
                .then((res) => {
                    setData(res.data.data)
                    const resValue = res.data.data.map(item => Number(item[1]))
                    setDataTemp(resValue)
